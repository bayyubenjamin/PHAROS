...
const PHAROS_PARAMS = {
  chainId: "0xa83b0", // 688848 in hex (REAL value returned by RPC)
...
function App() {
  ...
  // Fungsi connect wallet khusus untuk switch ke jaringan Pharos Testnet
  const connectWallet = async () => {
    ...
    try {
      ...
      await window.ethereum.request({
        ...
        params: [{ chainId: PHAROS_PARAMS.chainId }],
      });
    } catch (switchError) {
      if (switchError.code === 4902) {
        ...
        try {
          ...
          return alert(
            "Gagal menambahkan jaringan Pharos Testnet ke MetaMask:\n" +
              addError.message
          );
        }
      } else {
        ...
        return alert(
          "Gagal switch jaringan ke Pharos Testnet:\n" + switchError.message
        );
      }
    }
        ...
        {txHash && (
          <div className="mt-6 text-sm max-w-md break-words">
            ...
            <p className="mt-2">
              Contract:{" "}
              <a
                href={`https://testnet.pharosscan.xyz/address/${CONTRACT_ADDRESS}`}
                target="_blank"
                rel="noopener noreferrer"
